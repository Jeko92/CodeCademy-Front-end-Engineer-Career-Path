...
describe("", function () {
  it("", function () {
    ...
    expect(
      myExecutor,
      "`myExecutor()` should be a function."
    ...
    expect(
      logs[0] === resolvedValue,
      "Check the logic of your `myExecutor()` function. If the `sunglasses` property is greater than zero, `resolve()` should be invoked with `'Sunglasses order processed.'` Double check your spelling and punctuation."
    ).to.equal(true);
    ...
    expect(
      logs[1] === rejectValue,
      "Check the logic of your `myExecutor()` function. If the `sunglasses` property is zero, `reject()` should be invoked with `'That item is sold out.'` Double check your spelling and punctuation."
    ).to.equal(true);
  });
});
