...
function renderInputButtons(labels, groupName) {
  ...
  labels.forEach((label) => {
    // create the id attribute value for each input to emit the error message from browser about the violation of label and input rules
    let camelCaseLabel = label
      .replace(".", "")
      .split(" ")
      .map((word, index) => {
        return index === 0
    ...
    // give each input and camelCaseLabel as id to use it later as a value of 'for' attribute in respective labels
    inputElement.setAttribute("id", camelCaseLabel);
    divElement.appendChild(inputElement);

    // Create a label for that radio input element.
    let labelElement = document.createElement("label");
    labelElement.setAttribute("for", camelCaseLabel);
